...
* 마이페이지 조회 API 만들고... 지금까지 테스트코드 쭉 작성하다가, Swagger 써봤는데 세상에 너무 편하다!!! 클릭 한 번이면 된다니...!![[스크린샷 2025-01-23 오후 10.10.56.png]]
...
* 윤서님 postController에 에러 핸들링 핸들러로 하는 걸로 수정하기
* 내 memberController도...

* 나중에 반복되는 Validator는 어노테이션화해서 반복되는 코드 없도록 리팩토링하기
* 프로필 사진 설정 API에서, 유효한 URL인지 검증하는 로직 추가하기
	* 엥?? 갑자기 Bad Request 뜬다 따로 확인해보기 뭐지ㅠㅠ
		* 잉?? 프로필 설정 관련한 API에 다 Bad Request가 뜬다... 다시 확인해볼 것 -> 채팅 기능 만들고, 더미 데이터 넣은 다음, 내가 만든 API들 쭉 확인해보기
