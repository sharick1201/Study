...
* 테스트
	* 테스트환경의 다른 점
		**`memberProfileImageRepository.save()`가 `null`을 반환한 경우**
    
    - `Mock` 객체를 사용하는 테스트 환경에서는 `save()` 메서드가 명시적으로 반환값을 설정하지 않으면 `null`을 반환합니다.
    - 예를 들어, 아래와 같이 `when(memberProfileImageRepository.save(...)).thenReturn(...)`으로 반환값을 설정하지 않았을 경우 문제가 발생할 수 있습니다.
